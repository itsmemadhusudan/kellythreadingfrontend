...
router.post('/', authorize('admin'), async (req, res) => {
  try {
    ...
    const numSettlement = settlementAmount != null && settlementAmount !== '' ? Number(settlementAmount) : null;
    const pkg = await Package.create({
      name: String(name).trim(),
      price: numPrice,
      settlementAmount: numSettlement != null && !isNaN(numSettlement) && numSettlement >= 0 ? numSettlement : null,
    });
    res.status(201).json({
      success: true,
...
router.patch('/:id', authorize('admin'), async (req, res) => {
  try {
    ...
    if (settlementAmount !== undefined) {
      ...
      pkg.settlementAmount = num != null && !isNaN(num) && num >= 0 ? num : null;
    }
