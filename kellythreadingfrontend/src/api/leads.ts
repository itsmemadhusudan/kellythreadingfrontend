...
export async function getLeads(params?: { branchId?: string; status?: string; source?: string; serviceId?: string; dateFrom?: string; dateTo?: string }): Promise<{ success: boolean; leads?: Lead[]; message?: string }> {
...
export async function deleteLeads(ids: string[]): Promise<{ success: boolean; deletedCount?: number; message?: string }> {
  const r = await apiRequest<{ deletedCount: number }>('/leads/bulk-delete', { method: 'POST', body: JSON.stringify({ ids }) });
  if (r.success && 'deletedCount' in r) return { success: true, deletedCount: (r as { deletedCount: number }).deletedCount };
  return { success: false, message: (r as { message?: string }).message };
}
